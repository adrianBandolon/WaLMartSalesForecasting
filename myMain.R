...
update_forecast <-
  function(test_month, dept_preds, dept, num_model) {
...
update_test <- function(test_month) {
  test <<- test %>%
    ...
    select(
      -Weekly_Pred1.x,
      -Weekly_Pred1.y,-Weekly_Pred2.x,
      -Weekly_Pred2.y,-Weekly_Pred3.x,
      -Weekly_Pred3.y
    )
}
...
tslm_model <- function(train_ts, test_ts) {
  ...
  for (j in 2:ncol(train_ts)) {
    store_ts <- ts(train_ts[, j], frequency = 52)
    test_store_ts <- ts(test_ts[, j], frequency = 52)
    
    test_ts_temp <- tslm(store_ts ~ season + trend)
    test_ts[, j] <-
      forecast(test_ts_temp, newdata = test_store_ts, h = num_forecasts)$mean
  }
  test_ts
}
...
mypredict <- function() {
  ...
  for (dept in test_depts) {
    ...
    # SVD and keep the first 12 components
    train_dept_ts <- preprocess.svd(train_dept_ts, n.comp = 12)
    ...
    # naive forecast
    naive_mod <- naive_model(train_dept_ts, test_dept_ts)
    test_month <- update_forecast(test_month, naive_mod, dept, 1)
    
    snaive_mod <- snaive_model(train_dept_ts, test_dept_ts)
    test_month <- update_forecast(test_month, snaive_mod, dept, 2)
    
    tslm_mod <- tslm_model(train_dept_ts, test_dept_ts)
    test_month <- update_forecast(test_month, tslm_mod, dept, 3)
  }
